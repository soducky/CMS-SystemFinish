...
public class Thursday_Alarm : MonoBehaviour
{
    ...
    void Update()
    {
        if (DataManager.Instance.data.Alarm_weekday[4] == true && OpenAlarmSet && DateTime.Now > _alarmTime)
        {
        ...
        if (DataManager.Instance.data.Alarm_weekday[4] == true && CloseAlarmSet && DateTime.Now > Close_alarmTime)
        {
            AlarmMent.SetActive(true);
            GameObject.FindGameObjectWithTag("AlarmTimer").GetComponent<AlarmTimer>().Reset_Timer();
    ...
    {
        if (DataManager.Instance.data.Alarm_weekday[4] == true)
        {
            ...
            goalDay = Convert.ToInt32(DayOfWeek.Thursday);
    ...
    public void CloseSetAlarm()
    {
        if (DataManager.Instance.data.Alarm_weekday[4] == true)
        {
            ...
            Close_goalDay = Convert.ToInt32(DayOfWeek.Thursday);
